...
class TrainVizEvalVideo(EvalVideo):
    """
    Thin wrapper that runs the same forward/metric/viz logic as a base
    EvalVideo (e.g. HPTEvalVideo, PIEvalVideo) but writes its videos to
    ``<root>/videos_train_viz`` and prefixes its metric keys with
    ``train_viz/``. Used as the second val-loader evaluator when the data
    config defines ``train_viz_datasets``.
    """
