...
class ApplyArcTokenizer(Transform):
    """Thin data-transform adapter around the Arc Tokenizer MODULE.

    The algorithm itself lives in model format in
    ``egomimic.models.arc_tokenizer.ArcTokenizer`` (parameter-free nn.Module —
    the canonical implementation, shaped so learned variants can drop in). This
    adapter only routes per-sample batches through it inside dataloader workers,
    where tokenization runs in parallel on CPU. If the tokenizer gains learned
    parameters, drop this adapter and call the module in the trainer instead.

    Run zero-quat sanitization BEFORE this transform. Writes resampled chunks
    back under their keys and the (M, 1) velocity feature under ``speed_key``.
    """
    ...
    def __init__(
        ...
        from egomimic.models.arc_tokenizer import ArcTokenizer
        ...
        self.tokenizer = tokenizer or ArcTokenizer(
            delta_s=delta_s,
    ...
    def transform(self, batch: dict) -> dict:
        poses = {k: np.asarray(batch[k], dtype=np.float64) for k in self.pose_keys}
        points = {k: np.asarray(batch[k], dtype=np.float64) for k in self.point_keys}
        poses_out, points_out, speed = self.tokenizer.tokenize_window(
            poses, points, self.distance_keys
        )
        batch.update(poses_out)
        batch.update(points_out)
        batch[self.speed_key] = speed
        return batch
