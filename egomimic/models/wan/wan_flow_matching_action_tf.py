...
from egomimic.models.wan.base_action_head import ActionHead
from egomimic.models.wan.flow_match_scheduler import FlowMatchScheduler
from egomimic.models.wan.flow_unipc_multistep_scheduler import (
    FlowUniPCMultistepScheduler,
)
from egomimic.models.wan.vram_management import (
    AutoWrappedLinear,
    AutoWrappedModule,
    enable_vram_management,
)
from egomimic.models.wan.wan_video_text_encoder import T5LayerNorm, T5RelativeEmbedding
...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        if not config.skip_component_loading:
            ...
            print(
                "Skipping individual component loading (loading from full pretrained model)"
            )
    ...
    def forward(
        ...
        if actions.numel() > 0:
            assert (
                actions.min() >= -1.0 and actions.max() <= 1.0
    ...
    def lazy_joint_video_action(
        ...
        assert (
            self.clip_feas is not None and self.ys is not None
    ...
    def parallelize(self, device_mesh: DeviceMesh) -> None:
        ...
        assert (
            self.ip_rank >= 0 and self.ip_rank < self.ip_size
