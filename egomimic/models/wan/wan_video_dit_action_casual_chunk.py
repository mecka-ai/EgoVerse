...
from egomimic.models.wan.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from egomimic.models.wan.wan2_1_attention import AttentionModule
from egomimic.models.wan.wan2_1_submodule import (
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, e):
            # 3rd positional = context_lens for t2v cross-attn (None -> attend to
            # the full text context); harmlessly crossattn_cache=None for i2v.
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _prepare_blockwise_causal_attn_mask(
        ...
        assert (
            num_image_blocks == num_state_blocks
        ...
        padded_length = (
            math.ceil(
                (
                    local_attn_size * frame_seqlen
                    + (local_attn_size - 1)
                    + 32 * (local_attn_size - 1)
                )
                / 128
            )
            * 128
        ...
        def attention_mask(b, h, q_idx, kv_idx):
            ...
            # Image block query
            image_to_first = (
                q_is_image_block & kv_is_first_image
            )  # Image block to first image: always allowed
            image_to_image = (
                q_is_image_block & kv_is_image_block & (kv_block <= q_block)
            )  # Image block to image block: can attend to current and previous image blocks
            image_to_action = (
                q_is_image_block & kv_is_action & (kv_block == q_block)
            )  # Image block to action: can attend to current action block
            image_to_state = (
                q_is_image_block & kv_is_state & (kv_block == q_block)
            )  # Image block to state: can attend to current state block

            image_block_mask = (
                image_to_first | image_to_image | image_to_action | image_to_state
            )

            # Action query
            action_to_image = (
                q_is_action & kv_is_image_block & (kv_block <= q_block)
            )  # Action to image block: can attend to current and all previous image blocks
            action_to_action = (
                q_is_action & kv_is_action & (kv_block == q_block)
            )  # Action to action: only same block
            action_to_state = (
                q_is_action & kv_is_state & (kv_block == q_block)
            )  # Action to state: only same block
            action_to_first = (
                q_is_action & kv_is_first_image
            )  # Action to first image: always allowed

            action_mask = (
                action_to_image | action_to_action | action_to_state | action_to_first
            )
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            print("Created blockwise causal attention mask:")
    ...
    def _prepare_teacher_forcing_mask(
        ...
        padded_length = (
            math.ceil(self.local_attn_size * frame_seqlen / 128) * 128 - total_length
        )
        # padded_length = math.ceil(total_length / 128) * 128 - total_length
    ...
    def _prepare_blockwise_causal_attn_mask_i2v(
        ...
        # we do right padding to get to a multiple of 128
        padded_length = (
            math.ceil(local_attn_size * frame_seqlen / 128) * 128 - total_length
        )
        # padded_length = math.ceil(total_length / 128) * 128 - total_length
    ...
    def _forward_train(
        ...
        for block in self.blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                # block returns (x, updated_kv_cache); unpack (the checkpoint
                # branch does this in custom_forward). Without this, the
                # no-grad path (validation) passes a tuple to the next block.
                x, _ = block(x, **kwargs)
