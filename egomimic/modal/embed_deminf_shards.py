...
Two Modal functions run in parallel:
  _embed_state_shards  — L40S GPU, reads MP4s → StateEmbedder → per-episode state latents
  _embed_action_shards — CPU only, reads NPZs → ActionEmbedder → per-episode action latents
...
def _embed_state_shards(
    ...
    # episode_hash → list of (T, D) arrays (one per shard, but one shard = one episode here)
    ep_to_state: dict[str, list] = {}
    ...
    for mp4_path, npz_path in shard_pairs:
        ...
        T = len(frames_chw)
        if T == 0:
            print(f"{tag} {ep_hash[:8]}: 0 frames in MP4 — skipping")
            continue
...
def _embed_action_shards(
    ...
    """Embed action latents from NPZ shards. Returns path to action.npz."""
    import time as _time
    import numpy as _np
    import torch
    ...
    for _, npz_path in shard_pairs:
        try:
            npz = _np.load(npz_path, allow_pickle=True)
            ep_hash = str(npz["episode_hash"])
            actions = npz["action"].astype(_np.float32)  # (T, flat_dim)
        except Exception as exc:
            print(f"{tag} failed to read NPZ {npz_path}: {exc}")
            continue
