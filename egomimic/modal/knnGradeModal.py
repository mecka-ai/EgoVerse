...
(k, weights, thresholds…) re-runs CPU-only. Run outputs land in
    knn_grading/<name>/<description>_<timestamp>/
        ...
        scores_by_task.json       same content, viewer-compatible name
        scores_meta.json          {source, metric, higher_is_worse: true}
...
Tuning the prune (latent-viewer flow — preferred)
-------------------------------------------------
Export a t-SNE for the same data config with the scores attached, then tune
visually instead of watching videos off a CSV sheet:

    python egomimic/modal/latentVizModal.py data=<same data config> \\
        name=<viz run> description=<desc> \\
        scores_from=knn_grading/<name>/<description>_<ts>

In the served viewer: cluster the t-SNE (k-means), mark clusters
protect/normal/prune-first, drag the remove-% slider to preview the cut
(removed episodes dim; the video grid filters to removed/kept for
spot-checks), then export the keep-list and apply it with
``egomimic/scripts/knn_grading/review_tools.py apply`` to get an
``eps_to_use`` allowlist. The CSV sheet + ``calibrate`` flow in
review_tools.py still works for label-based threshold validation.
...
def run_knn_grade(
    ...
    # Viewer-compatible duplicates: the latent viewer auto-pairs a run dir's
    # scores_by_task.json, and scores_meta.json tells it the score direction
    # (knn primary_score is worst-first — DemInf KSG scores are the opposite).
    with open(output_dir / "scores_by_task.json", "w") as f:
        json.dump(scores_by_task, f, indent=2)
    with open(output_dir / "scores_meta.json", "w") as f:
        json.dump(
            {
                "source": "knn_grading",
                "metric": "primary_score",
                "higher_is_worse": True,
                "run": str(output_dir.relative_to(CFG.output_mount_path)),
            },
            f,
            indent=2,
        )
...
def submit_knn_grade(*hydra_args: str) -> None:
    ...
    Submodules default OFF: grading never imports openpi/lerobot, and
    initializing them would downgrade transformers below DINOv3 support.
    """
