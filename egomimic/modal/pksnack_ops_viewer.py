"""Web viewer for the packaging_snacks operator-diversity experiment (pksnack_ops).

Four levels (L1..L4) spread a fixed ~10 h data budget over 8 / 24 / 72 / 160
operators, all on the 300M model. Each level runs THREE validation passes, and
each pass renders its own GT-vs-pred video family every 30 epochs. The dir names
below were read off the runs' own .hydra/config.yaml on the volume:

    videos/           valid_datasets = pksnack_opscale_val_indom8.json
                      in-domain-operator holdout (8 episodes, operators seen in
                      training, episodes never trained on)
    videos_oph/       train_viz_evaluator (video_dirname: videos_oph)
                      = pksnack_val_ophold5.json — held-out operator (5 episodes,
                      a person the model never saw)
    videos_trainviz/  extra_val_evaluator (video_dirname: videos_trainviz,
...
Full path pattern (see egomimic/eval/eval_video.py):
    pksnack_ops/L<n>/<family>/epoch_<N>/MECKA_BIMANUAL/validation_video_<i>.mp4

Deploy:
    MODAL_ENVIRONMENT=robotics modal deploy egomimic/modal/pksnack_ops_viewer.py

Routes (single ASGI web function → one URL):
    /            HTML viewer page (up to 12 rows = 4 levels x 3 val families)
    /api/index   {"rows": [...], "epochs": [...]} — volume scan, cached 60 s
    /video?path= streams one mp4 from the volume (path relative to pksnack_ops/)
"""
...
app = modal.App("pksnack-ops-viewer", image=image)

outputs_volume = modal.Volume.from_name("egoverse-training-outputs")

MOUNT_PATH = "/data"
BASE_PREFIX = "pksnack_ops"
INDEX_TTL_S = 60

PAGE_TITLE = "pksnack_ops — operator diversity val videos"
PAGE_HEADING = "pksnack_ops — operator diversity"

# (group dir, group caption) — one card per level.
GROUPS = [
    ("L1", "8 operators"),
    ("L2", "24 operators"),
    ("L3", "72 operators"),
...
GROUP_SUFFIX = "· ~10 h budget · 300M"

# (subdir, short label, description, css class) — one row per (group, family).
FAMILIES = [
    ("videos", "in-domain-operator holdout", "8 eps · operators seen, episodes unseen", "f0"),
    ("videos_oph", "held-out-operator val", "5 eps · operator never seen", "f1"),
    ("videos_trainviz", "train-viz", "4 eps · trained on (sanity check)", "f2"),
]

BLURB_HTML = """
    Operator-diversity scaling on <b>packaging_snacks</b> at a
    <b>fixed ~10 h data budget</b> with the <b>300M</b> model: L1/L2/L3/L4 spread
    the same amount of data over <b>8 / 24 / 72 / 160 operators</b>. Three val
    passes per level —
    <span class="swatch sw-f0"></span><b>in-domain-operator holdout</b>: unseen
    episodes from operators that ARE in the training mix (measures fit);
    <span class="swatch sw-f1"></span><b>held-out-operator val</b>: an operator
    the model never trained on (measures transfer to a new person — the metric
    this experiment is really about);
    <span class="swatch sw-f2"></span><b>train-viz</b>: episodes the model DID
...
@modal.asgi_app(label="pksnack-ops-viewer")
def viewer():
    import os
    import re
    import threading
